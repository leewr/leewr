      ...
      return (
        <div className="citem" key={index}>
          <div className="chead">
            <img
              alt=""
              src="http://upload.jianshu.io/users/upload_avatars/3334769/95a8e26b-bb13-4d93-92b3-2e504cf66237.jpg?imageMogr2/auto-orient/strip|imageView2/1/w/96/h/96"
            />
            <span className="name">{item.authorName}</span>
            <span className="time">
              {dateFtt(new Date(item.modifyTime), 'yyyy.MM.dd hh:mm')}
            </span>
          </div>
          <div className="cbody">{item.content}</div>
          <div className="cfoot">
            {/* <span>点赞{item.thumbs}</span> */}
            <span onClick={this.toggleThumbs.bind(this, index, item.cid)}>
    ...
    return (
      <div className="comment-panel">
        ...
        {this.props.commentData.length > 0 ? (
          ...
          <h3 className="noComment">你会是打破沉默的第一个人么？</h3>
        )}
      </div>
    )
  }
