...
class TopicService extends Service {
  ...
  async getArticleList(pagination, authorId) {
    ...
    if (authorId) {
      ...
      result = await this.app.mysql.query(
        'select article.authorId,article.cid,article.commentNum,article.id, article.imgUrl,article.summary,article.title,article.view, user.name, user.avatarUrl from article left join user on article.authorId = user.id order by modifyTime desc, id desc limit ? offset ?',
  ...
  async addView(id) {
    return await this.app.mysql.query(
      'update article set view = (view + ?) where id = ?',
      [1, id]
    )
  }
  ...
  async toggleLike(articleId, current_user) {
    ...
    // 文章喜欢表中查询 不存在 新增一条记录
    let status = await this.app.mysql.query(
      'update thumbs set status = !status, modifyTime = now() where articleId = ? and userId = ?',
      [articleId, current_user.id]
    )
    if (!status.changedRows) {
      status = await this.app.mysql.query(
        'insert into thumbs values(0, ?, ?, now(), now(), 1)',
        [articleId, current_user.id]
      )
    }
    ...
    await this.app.mysql.query(
      'update article set likeNum = (likeNum + ?) where id = ?',
      [data.status ? 1 : -1, articleId]
    )
    ...
    await this.app.mysql.query(
      ...
      [data.status ? 1 : -1, article.authorId]
    )
  ...
  async topArticle(day = 7, authorId) {
    return await this.app.mysql.query(
      'select * from article where to_days(now()) - to_days(createtime) < ? and authorId = ? order by view desc',
      [day, authorId]
    )
  }
  ...
  async likedAeticle(userId) {
    return await this.app.mysql.query(
      'select * from article where id in (select articleId from thumbs where userId = ? and status = 1)',
      userId
    )
  }
}
